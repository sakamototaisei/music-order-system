...
const genreData = [
    ...
    {
        "name": "R&B / ソウル",
        "subgenres": ["クラシックソウル", "コンテンポラリーR&B", "ニューソウル", "モータウン", "ファンク"]
    },
    {
        "name": "ダンス / ディスコ",
...
export default function MusicOrderForm({ user, onOrderCreated }: MusicOrderFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('music_orders').insert(orderData);

    if (error) {
      setMessage(`エラー: ${error.message}`);
    } else {
      setMessage('音楽オーダーを正常に作成しました！');
      setTheme('');
      setInstruments('');
      setHasLyrics(false);
      setLyricsContent('');
  ...
  return (
    <div className="w-full max-w-2xl p-8 space-y-6 bg-white rounded-lg shadow-md">
      <h2 className="text-xl font-bold text-gray-900">新規音楽オーダー作成</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-2">
            <label htmlFor="otherGenre" className="text-sm font-medium text-gray-700">
              その他 (自由入力)
            </label>
            <input
              id="otherGenre"
        ...
        <div className="flex items-center">
          ...
          />
          <label htmlFor="hasLyrics" className="ml-2 block text-sm text-gray-900">
            歌詞あり
          </label>
        </div>
        ...
        {hasLyrics && (
          <div>
            <label htmlFor="lyricsContent" className="text-sm font-medium text-gray-700">
              歌詞の内容
            </label>
            <textarea
              id="lyricsContent"
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-gray-900"
            placeholder="参考曲、イメージなど自由にご記入ください"
          />
        </div>
